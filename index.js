...
if(!outfile[1]){
    outfile[1] = `${path.dirname(filepath)}/mockoon_${path.posix.basename(filepath)}`;
}
...
const { Item, Mockoon, Response, Route, Environment } = require('./lib/models/');;
    ...
    collection.item.forEach(r_data=>{
        ...
        if(r_data.response.length>0)
        {
            r_data.response.forEach(r=>{       
                ...
                if(r.code){
                    resp.statusCode = rawdata.code.toString();
                }
                if(r.header){
